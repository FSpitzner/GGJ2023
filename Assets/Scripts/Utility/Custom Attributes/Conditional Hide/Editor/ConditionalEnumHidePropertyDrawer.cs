...
public class ConditionalEnumHidePropertyDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (!condHAtt.InBetween)
        {
            enabled = (condHAtt.EnumValue1 == enumValue) || (condHAtt.EnumValue2 == enumValue);
            GUI.enabled = enabled;
		...
		{
            enabled = (enumValue >= condHAtt.EnumValue1) && (enumValue <= condHAtt.EnumValue2);
            GUI.enabled = enabled;
            if(!condHAtt.HideInInspector || enabled)
			{
                EditorGUI.PropertyField(position, property, label, true);
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        else if (!condHAtt.HideInInspector || (enumValue >= condHAtt.EnumValue1) && (enumValue <= condHAtt.EnumValue2))
		{
            return EditorGUI.GetPropertyHeight(property, label);
		}
        else
